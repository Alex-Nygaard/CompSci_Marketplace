...
blueprint = make_google_blueprint(client_id="872204614354-41hm7nk3jb89fsrtj4madroi510npoq5.apps.googleusercontent.com",
                                  client_secret="mitMmKkTn15WQSxIkiBTMzu3", offline=True, scope=["profile", "email"],
...
# Custom validation method for expiration date field
...
def validate_date(form, field):
    ...
    if len(date[0]) != 4 or len(date[1]) != 2 or len(date[2]) != 2:
        raise ValidationError("Please submit the date in the form 'DD-MM-YYYY'.")
"""
...
def index():
    ...
    if form.validate_on_submit():  # When user clicks "search" this is True
        ...
        s_items_description = Item.query.filter(
            Item.description.contains(search)).all()  # Queries DESCRIPTIONS of items that includes search term, returns list
...
def sell():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            img.save(os.path.join(os.getcwd(), "static", "item_pics", str(current_user.id) + filename))
...
def profile():
    ...
    return render_template("profile.html", google=google, all_users=all_users, current_user_items=current_user_items, all_items=all_items, transactions=transactions,current_date=datetime.now())
